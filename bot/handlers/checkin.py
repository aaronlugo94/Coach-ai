...
async def handle_ci(query, uid: int, context):
    """
    Procesa el check-in nocturno de 2 taps.
    ...
    # Acumulamos en user_data hasta tener ambas respuestas
    ...
    # Si ya tenemos ambas respuestas → generar análisis
    if "rutina" in ci and "dieta" in ci:
        context.user_data["checkin"] = {}  # reset
        await _procesar_completo(query, uid, context, ci)
    else:
        # Confirmar el tap y esperar el segundo
        tap = "✅" if tipo == "rutina" else "🥗"
        try:
            await query.edit_message_text(
                query.message.text + f"\n\n{tap} Registrado — toca la segunda opción 👆",
